...
class MLModelFamily:
    ...
    def add_CL_param(self, cl_alg, num_clusters):
        self.models[cl_alg][num_clusters] = {}
        self.data[cl_alg][num_clusters] = {}

    def del_CL_param(self, cl_alg, num_clusters):
        del self.models[cl_alg][num_clusters]
        del self.data[cl_alg][num_clusters]

    def add_CL_model(self, cl_alg, num_clusters, cl):
        if 'CL_MODEL' not in self.models[cl_alg][num_clusters]:
            self.models[cl_alg][num_clusters]['CL_MODEL'] = {}

        self.models[cl_alg][num_clusters]['CL_MODEL'] = cl

    def set_CL_model_avg_r2(self, cl_alg, num_clusters, avg_r2):
        self.models[cl_alg][num_clusters]['AVG_R2'] = avg_r2

    def get_CL_model_avg_r2(self, cl_alg, num_clusters):
        return self.models[cl_alg][num_clusters]['AVG_R2']
    ...
    def set_CL_data(self, cl_alg, num_clusters, cur_cluster, data_name, data):
        if cur_cluster not in self.data[cl_alg][num_clusters]:
            self.data[cl_alg][num_clusters][cur_cluster] = {}
        self.data[cl_alg][num_clusters][cur_cluster][data_name] = data

    def add_SL(self, cl_alg, num_clusters, cur_cluster, prediction_alg):
        if cur_cluster not in self.models[cl_alg][num_clusters]:
            self.models[cl_alg][num_clusters][cur_cluster] = {}

        self.models[cl_alg][num_clusters][cur_cluster][prediction_alg] = {}

    def del_SL(self, cl_alg, num_clusters, cur_cluster, prediction_alg):
        del self.models[cl_alg][num_clusters][cur_cluster][prediction_alg]

    def add_SL_model(self, cl_alg, num_clusters, cur_cluster, prediction_alg, model):
        self.models[cl_alg][num_clusters][cur_cluster][prediction_alg]['SL_MODEL'] = model

    def set_SL_model_r2(self, cl_alg, num_clusters, cur_cluster, prediction_alg, r2):
        if math.isnan(r2):
            ...
            self.models[cl_alg][num_clusters][cur_cluster][prediction_alg]['R2'] = -10
        else:
            self.models[cl_alg][num_clusters][cur_cluster][prediction_alg]['R2'] = r2

    def get_SL_model_r2(self, cl_alg, num_clusters, cur_cluster, prediction_alg):
        return self.models[cl_alg][num_clusters][cur_cluster][prediction_alg]['R2']

    def get_CL_models(self, cl_alg, num_clusters):
        cl_models = {}
        for key, value in self.models[cl_alg][num_clusters].items():
...
class DataClusterBasedMachineLearning:
    ...
    def __init__(self, data_x, data_y, clustering_variables, clustering_algs, prediction_algs, max_clusters, metrics='R2'):
        """
    ...
    def perform_machine_learning_alg(self, cl_alg, num_clusters, cur_cluster, prediction_alg, x_clustered, y_clustered):
        """
        This performs an SL algorithm
        :param cl_alg: The name of a clustering algorithm
        :param num_clusters: The maximum number of the clustering algorithm
        ...
        cbn_name = f'{cl_alg}_{num_clusters}_{cur_cluster}_{datetime.now().strftime("%m_%d_%Y %H_%M_%S")}'
        n_splits = 2
        ...
        self.ml_family.add_SL_model(cl_alg, num_clusters, cur_cluster, prediction_alg, model)
        self.ml_family.set_SL_model_r2(cl_alg, num_clusters, cur_cluster, prediction_alg, r2_avg)

    def perform_machine_learning(self, cl_alg, num_clusters, cur_cluster, x_clustered, y_clustered):
        """
        ...
        for prediction_alg in self.prediction_algs:
            self.ml_family.add_SL(cl_alg, num_clusters, cur_cluster, prediction_alg)

            print(f'[Thread] {cl_alg}.{num_clusters}.{cur_cluster} -> ML alg {prediction_alg}')

            t = threading.Thread(target=self.perform_machine_learning_alg, args=([cl_alg, num_clusters, cur_cluster, prediction_alg, x_clustered, y_clustered]))
            t.setDaemon(True)
            thread.append(t)
        ...
        for prediction_alg in self.prediction_algs:
            r2 = self.ml_family.get_SL_model_r2(cl_alg, num_clusters, cur_cluster, prediction_alg)

            print(f'Check R2 for {cl_alg}.{num_clusters}.{cur_cluster}.{prediction_alg} = {r2}')
            r2_high = min_or_max(r2_high, r2)
            if r2 is r2_high:
                if alg_high is not None:
                    self.ml_family.del_SL(cl_alg, num_clusters, cur_cluster, alg_high)
                alg_high = prediction_alg
            else:
                self.ml_family.del_SL(cl_alg, num_clusters, cur_cluster, prediction_alg)
    ...
    def perform_clustering_alg_with_clusters(self, cl_alg, num_clusters):
        ...
        :param num_clusters: A maximum number of clusters
        """
        ...
        self.ml_family.add_CL_model(cl_alg, num_clusters, cl)
        ...
        for cur_cluster, datum in data_x.items():
            print(f'data split for {cl_alg}.{num_clusters}.{cur_cluster} X-Size[{len(data_x[cur_cluster])}] Y-Size[{len(data_y[cur_cluster])}]')

            # inconsistent clustered data!
            if len(data_x[cur_cluster]) != len(data_y[cur_cluster]):
                warnings.warn('inconsistent clustered data!')
                self.ml_family.set_CL_model_avg_r2(cl_alg, num_clusters, -10000)
                return
        ...
        for cur_cluster, datum in data_x.items():
            ...
            if self.is_experiment:
                ...
                self.ml_family.set_CL_data(cl_alg, num_clusters, cur_cluster, 'x_train', data_x[cur_cluster])
                self.ml_family.set_CL_data(cl_alg, num_clusters, cur_cluster, 'y_train', data_y[cur_cluster])

            t = threading.Thread(target=self.perform_machine_learning, args=([cl_alg, num_clusters, cur_cluster, data_x[cur_cluster], data_y[cur_cluster]]))
            t.setDaemon(True)
            thread.append(t)
        ...
        #############################################################################
        # 4. Calculate the average R2 and store it to 'ml_models.cl_alg.num_clusters.avg_r2'
        avg_r2 = []
        cl_models = self.ml_family.get_CL_models(cl_alg, num_clusters)
        ...
        self.ml_family.set_CL_model_avg_r2(cl_alg, num_clusters, mean_avg_r2)
    ...
    def perform_clustering(self, cl_alg):
        ...
        for num_clusters in range(2, self.max_clusters + 1):
            self.ml_family.add_CL_param(cl_alg, num_clusters)
        ...
        for num_clusters in range(2, self.max_clusters + 1):
            avg_r2 = self.ml_family.get_CL_model_avg_r2(cl_alg, num_clusters)

            print(f'Check avg_r2 for {cl_alg}.{num_clusters} = {avg_r2}')
            ...
            if avg_r2 is avg_r2_high:
                ...
                self.ml_family.del_CL_param(cl_alg, num_clusters)
    ...
    def get_cl_model(self):
        ...
        if isinstance(cl_model['CL_MODEL'], Clustering_Alg):
            return cl_model['CL_MODEL'].get_selected_clustering_alg()
        else:
            return None
