...
test = test[ test.Date >=  '2020-03-25']test
# In[93]:
...
train[train.Province_State == 'California']import sys
...
contain_data.columnsdataset.groupby('Country').S_data_days.max().sort_values(ascending = False)[-30:]contain_data.StringencyIndex.cummax()contain_data.groupby('Date').count()[90:]
# In[114]:
...
sup_data.drop(columns = ['longitude', 'temperature', 'humidity',
                        'latitude'], inplace=True)sup_data.columnssup_data.drop(columns = [c for c in sup_data.columns if 
                                 any(z in c for z in ['state', 'STATE'])], inplace=True)sup_data = sup_data[['Province_State', 'Country_Region',
                     'Largest_City',
                     'IQ', 'GDP_region', 
...
# old_path = path + 'others_prior_predictions/wk2_input'
# old_test = pd.read_csv(old_path + '/' + 'test.csv')
# old_train = pd.read_csv(old_path + '/' + 'train.csv')


# In[145]:


# prior_predictions = pd.read_excel(path + '/' + 'wk2-predictions.xlsx',
...
# prior_predictions = pd.merge(prior_predictions, old_test, on = 'ForecastId')


# In[147]:


# prior_predictions.Date = pd.to_datetime(prior_predictions.Date)
# prior_predictions.rename(columns={'Country_Region': 'Country'}, inplace=True)
# prior_predictions['Place'] = prior_predictions.Country\
#                             + prior_predictions.Province_State.fillna("")

# old_train.Date = pd.to_datetime(old_train.Date)
# old_train.rename(columns={'Country_Region': 'Country'}, inplace=True)
# old_train['Place'] = old_train.Country\
#                             + old_train.Province_State.fillna("")
...
# tpo = pd.merge(train, 
#                             prior_predictions[(prior_predictions.Date > old_train.Date.max())]
#                                  , on = ['Date', 'Place'])
...
def placeScores(df):
    df = df.copy()
    df['ce'] = np.power( np.log(df.ConfirmedCases_x + 1) - np.log(df.ConfirmedCases_y + 1), 2)
    df['fe'] = np.power( np.log(df.Fatalities_x + 1) - np.log(df.Fatalities_y + 1), 2)
    df['se'] = df.ce + df.fe
    return df.groupby('Place')[['ce', 'fe', 'se']].mean().sort_values('se', ascending = False)

tpo[tpo.Place=='New Zealand']
# In[ ]:
...
# np.median(ps)
# np.round(ps.sort_values('se', ascending=False), 3)
...
# drop_places = ps[ps.se  > PS_CUTOFF * np.random.rand(len(ps)) * 2].index
# drop_places
...
# len(drop_places)/len(ps)

np.sqrt(placeScores(train_predict_overlap).mean())
# In[154]:


# prior_predictions = prior_predictions[~prior_predictions.Place.isin(drop_places)]

len(prior_predictions.Place.unique())
# In[155]:


# prior_predictions = prior_predictions[(prior_predictions.Date > old_train.Date.max())]

prior_predictions.Date.unique()train_bk
# In[156]:
...
if USE_PRIORS:
    train = train[     ( train.Place.isin(prior_predictions.Place.unique()) &
                                 ( train.Date < prior_predictions.Date.min() )   )  |
                      ( ~ train.Place.isin(prior_predictions.Place.unique())) ]

    prior_predictions.rename(columns = {'ForecastId': 'Id'}, inplace = True)



    train = pd.concat( (train, prior_predictions[train.columns] )).sort_values(['Place', 'Date'])

    assert len(train) - len(train_bk) - len(prior_predictions)         == -len(prior_predictions.Place.unique() )                 * ( (train_bk.Date.max() - prior_predictions.Date.min()).days + 1 )
...
lplot( train_nc, REAL)train[train.Province_State=='California']lplot( train_nc[train_nc.Province_State.isnull()])
# In[ ]:




lplot( train_nc[train_nc.Province_State.isnull()], REAL)lplot( train_nc[~train_nc.Province_State.isnull()], REAL)lplot(train[train.Country=="US"])
# In[ ]:
...
# ##### CFR
np.log( 0 + 0.015/1)BLCFR = -4.295015257684252
# In[198]:
...
mean_7d_c_slope.plot()dataset.columns[:100]mean_7d_c_slope.plot()date_totals.Fatalities_7d_prior_slope.plot()date_counts = dataset.groupby('Date').apply(lambda x:  x > 0)date_countsdate_totals['world_cases_chg'] = (np.log(date_totals.ConfirmedCases + 1 )\
...
dataset[dataset.Country=='China'][['Place', 'Date', 
               'ConfirmedCases_percapita_vs_continent_mean',
               'Fatalities_percapita_vs_continent_mean']][1000::10]dataset[['Place', 'Date', 
...
dataset[dataset.ConfirmedCases>0]['log_cfr'].plot(kind='hist', bins = 250)dataset.log_cfr.isnull().sum()
# In[222]:
...
data[slope_cols][c for c in data.columns if any(z in c for z in [ 'rate']) ]
# In[233]:
...
len(data)data[(data.ConfirmedCases_7d_prior_slope < .01) & (data.ConfirmedCases > 20)
        & (data.days_since_first_case > 20)
    ].groupby('Country').count()len(data[data.ConfirmedCases > 1])
# In[ ]:
...
np.std(x.log_cases)np.std(x.log_fatalities)
# In[ ]:
...
for c in data.columns.to_list():
    if 'trendline_per' in c:
        ...
        h = plt.figure()
        print(np.std(x[c]))[(c, np.min(x[c]), np.max(x[c])) for c in x.columns.to_list()
    if  np.max(x[c]) - np.min(x[c])  > 20]for c in x.columns.to_list()[20:25]:
    print(c)
    pf= x[c].plot(kind = 'hist', bins = 250);
    pf = plt.figure()x_full = data[  x.columns].copy()x_full.shape
# In[ ]:
...
data_test[data_test.Place=='San Marino'].Date_fdata_test.groupby('Place').Date_f.count().sort_values()x_test
# In[ ]:
...
#                                  cv = GroupKFold(4),
        ...
        if verbose > 0:
            print(pd.DataFrame(clf.cv_results_)); print();  
...
# return x;[c for c in x.columns if any(z in c for z in 
                                 ['prior_slope', 'prior_value'])]getSparseColumns(x, verbose = 0)x.columns[::19]
# In[295]:
...
[c for c in x.columns if 'continent' in c]x.columns[::10]x.shapecontain_data.columns
# In[297]:
...
lgb_f_clfs = lgb_f_clfs[0:3]lgb_f_noise = lgb_f_noise[2:]
lgb_f_clfs = lgb_f_clfs[2:]et_f_clfs = runBags(x, y_fatalities, groups, MSE, 1, trainETR, verbose = 1)
...
fis = linear_FI_plot(fis, x.columns.values,25)lgb_c_clfs
# In[313]:
...
x.days_since_Stringency_1.plot(kind='hist', bins = 100)len(x.log_fatalities.unique())
# In[317]:
...
f = avg_FI([et_c_clfs], x.columns, 25)f = avg_FI([et_f_clfs], x.columns, 25)
# In[ ]:




np.corrcoef((x.S_data_days.fillna(0), y_cases, y_fatalities, y_cfr))np.corrcoef((x.murder.fillna(0), y_cases, y_fatalities, y_cfr))np.corrcoef((x.Personality_uai, y_cases, y_fatalities, y_cfr))np.corrcoef((x.TFR, y_cases, y_fatalities, y_cfr))np.corrcoef((x.Avg_age, y_cases, y_fatalities, y_cfr))np.corrcoef((x.High_rises, y_cases, y_fatalities, y_cfr))np.corrcoef((x.max_high_rises, y_cases, y_fatalities, y_cfr))data.elapsedfi_list = []
for clf in enet_f_clfs:
    fi = clf.coef_ * np.std(x, axis=0).values 
    fi_list.append(fi)
fis = np.mean(np.array(fi_list), axis = 0)
fis = linear_FI_plot(fis, x.columns.values,25)datashow_FI(lgb_f_clf, x.columns, 25)
# In[ ]:
...
cfr_predslgb_cfr_noiselgb_cfr_clfs[0].predict(noisify(x_test, 0.4))cfr_preds[0][0:500]x.log_cfr.plot(kind='hist', bins = 250)
# In[ ]:
...
data_test.shapepd.DataFrame({'c_mean': np.mean(c_preds, axis =0 ),
             ...
             }, index=data_test.Place)[::7]np.median(c_preds, axis =0 )[::71]
# In[ ]:
...
MAX_DATE = np.max(train.Date)final = train[train.Date == MAX_DATE]
# In[ ]:
...
train.groupby('Place')[['ConfirmedCases','Fatalities']].apply(lambda x: np.sum(x >0))num_changes = train.groupby('Place')[['ConfirmedCases','Fatalities']].apply(lambda x: np.sum(x - x.shift(1) >0))num_changes.Fatalities.plot(kind='hist', bins = 50);num_changes.ConfirmedCases.plot(kind='hist', bins = 50);
# In[ ]:
...
def getRate(train, window = 5):
    ...
    return ratesltr = getRate(train, 14)lm = pd.merge(ltr, num_changes, on='Place')lm.filter(like='China', axis='rows')flat = lm[
    (lm.CasesRate < 0.01) & (lm.ConfirmedCases > 5)]flatc_rate = pd.Series(
...
m['CaseChange'] = (np.log(m.ConfirmedCases_y + 1) - np.log(m.ConfirmedCases_x + 1))/7m[m.Place=='USMaine']
# In[ ]:
...
# ##### CFR Charts
joined.Fatalities_ywithcases = joined[joined.ConfirmedCases_y > 300]withcases.sort_values(by = ['Fatalities_y'])(withcases.Fatalities_y / withcases.ConfirmedCases_x).plot(kind='hist', bins = 150);(final.Fatalities / final.ConfirmedCases).plot(kind='hist', bins = 250);
# In[ ]:
...
test_wp[test_wp.Country == 'US']test_wp
# In[358]:
...
assert len(full) == len(data_wp)x_test.shape
# In[389]:
...
full.Fatalities =  np.where( (full.Fatalities > full.ConfirmedCases * CFR_CAP) &
                            ...
                            , 
                                               full.Fatalities)train[train.Country=='Italy']final[final.Country=='US'].sum()
# In[397]:
...
high_final_slope = final_slope[final_slope > 0.1].indexslope_change = (ffm.groupby('Place').last().case_slope - ffm.groupby('Place').first().case_slope)
...
test.Date.min()set(high_slope_increase) & set(high_final_slope)ffm.groupby('Date').case_slope.median()
# In[ ]:
...
ffc[(np.log(ffc.Fatalities_x) - np.log(ffc.ConfirmedCase_x)) / ffc.elapsed_y ]ffm.groupby('Place').case_slope.last().sort_values(ascending = False)[:30]lplot(test_wp)lplot(test_wp, columns = ['case_slope', 'fatality_slope'])
# In[ ]:
...
prior_best = pd.read_csv(path + '/output/final_blend_1' +'/'  +'submission_basic_lgb_joined.csv')prior_best_full = pd.merge(prior_best, test, on='ForecastId')combo_prior_best = pd.merge(full_train, prior_best_full, 
                                on=['Place', 'Date'], suffixes = ['_true', '_pred'])
combo_prior_best.set_index(['Place', 'Date'], inplace=True)combo_prior_best_scored = RMSLE(combo_prior_best, verbose = 0)
...
# ALL OF THE ERROR COMES FROM THINGS THAT RAMP TOO QUICKLY
...
).sort_values('ConfirmedCases', ascending=False)ffm[ffm.Date == test.Date.max()].groupby('Place').agg(
    {'ForecastId': 'count',
     'case_slope': 'mean',
        'fatality_slope': 'mean',
            'ConfirmedCases': 'sum',
...
Libya                                 0.48625
Suriname                               0.4375
United Kingdom                     0.26053571
Guyana                                0.24875
Guinea-Bissau                         0.24375
# In[ ]:
...
Morocco                0.625
Kazakhstan            0.5425
Dominican Republic   0.50875

Latin American ramp-ups...
# In[ ]:
...
for file_suffix in ['controlled_joined' , 'extended_joined']:
    ...
    if len(sets):
        ...
        r1 =predicted[predicted.Date == predicted.Date.max()].groupby('Country')                    [cols].apply(lambda x: (np.exp(x) -1 ).sum())                .sort_values('ConfirmedCases', ascending = False)
        r1.head()
        r1.tail()

        r2 = predicted[predicted.Date == predicted.Date.max()].groupby('Country')                    [cols].apply(lambda x: (np.exp(x) -1 ).sum())                .sort_values('Fatalities', ascending = False)
        r2.head()
        r2.tail()
...
i_joined = pd.merge( pred[(pred.Country=='US') & (pred.Date > train_bk.Date.max())], 
         ...
         .rename(columns = {'location': 'Province_State', 
                        ...
                        how = 'inner', validate = '1:1')i_joined['elapsed'] = (i_joined.Date - train_bk.Date.max()).dt.days predicted = i_joined.copy()cols = ['ConfirmedCases', 'Fatalities', 'IHME_f']
for col in [c for c in predicted.columns.to_list() if any(z in c for z in cols)]:
    predicted[col] = np.log(predicted[col] + 1 )predicted['gap'] = (predicted.IHME_f - predicted.Fatalities)\
            * np.clip(predicted.elapsed / 25, 0, 1)predicted.gap = np.where(predicted.gap < 0,
                            predicted.gap * 0.7, 
                                predicted.gap)predicted['Fatalities_a'] = predicted.Fatalities + predicted.gap * 0.43
predicted['ConfirmedCases_a'] = predicted.ConfirmedCases + predicted.gap * 0.2
cols_2 = cols + ['ConfirmedCases_a', 'Fatalities_a']np.round(predicted[predicted.Date == predicted.Date.max()].groupby('Place')\
            [cols_2 +['gap']].apply(lambda x: (np.exp(x) -1 ).sum())\
        .sort_values('ConfirmedCases', ascending = False),1)[:51]adj_pred = predicted.copy()for col in ['ConfirmedCases', 'ConfirmedCases_a', 'Fatalities', 'Fatalities_a', 'IHME_f']:
    adj_pred[col] = np.exp(adj_pred[col]) - 1np.round(adj_pred.groupby('Place').last(),1)np.round(adj_pred.groupby('Date').sum(),1)adj_pred[adj_pred.Date==adj_pred.Date.max()].sum()##### Outputcols = ['ConfirmedCases', 'Fatalities']adj_pred = adj_pred[['ForecastId', 'Fatalities_a', 'ConfirmedCases_a']]\
