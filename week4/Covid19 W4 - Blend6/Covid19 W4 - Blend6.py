...
class CovidModelAhmet(CovidModel):
    ...
    def __init__(self):
        ...
        Country_Region = ['US']
        Province_State = ['Guam']
        df = df.loc[df['Country_Region'].isin(Country_Region)]
        df = df.loc[df['Province_State'].isin(Province_State)]

        # Select training days
        mask = (df['Date'] >= '2020-01-22') & (df['Date'] <= '2020-04-15')
        df = df.loc[mask]
...
class CovidModelCPMP(CovidModel):
    ...
    def __init__(self):
        ...
        # Select training days
        mask = (train['Date'] >= '2020-01-22') & (train['Date'] <= '2020-04-15')
...
# %% [code]
## defining constants
PATH_TRAIN = "../input/covid19-global-forecasting-week-4/train.csv"
PATH_TEST = "../input/covid19-global-forecasting-week-4/test.csv"
...
# The first day when a case of 1~10000 occurs
...
region_metadata = region_metadata.loc[region_metadata['Country_Region'].isin(Country_Region)]
region_metadata = region_metadata.loc[region_metadata['Province_State'].isin(Province_State)]
region_date_metadata = region_date_metadata.loc[region_date_metadata['Country_Region'].isin(Country_Region)]
region_date_metadata = region_date_metadata.loc[region_date_metadata['Province_State'].isin(Province_State)]
...
n_dates_train = len(df_panel[~df_panel.ConfirmedCases.isna()].Date.unique())
...
df = df.loc[df['Country_Region'].isin(Country_Region)]
df = df.loc[df['Province_State'].isin(Province_State)]
sub_df = sub_df.loc[sub_df['Country_Region'].isin(Country_Region)]
sub_df = sub_df.loc[sub_df['Province_State'].isin(Province_State)]
...
weights = {"ahmet": 0.65, "giba1": 0.175, "giba2": 0.175}
