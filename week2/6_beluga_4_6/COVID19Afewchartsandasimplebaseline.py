...
# # Summary
#
# **Disclaimer** We still have limited input to predict or understand what will happen in the next few weeks (months).
#
# At this point I see more value in collecting input and monitoring the outbreak than trying to predict the future.
#
...
#  * Most countries have only 2 weeks input
...
# %% [code] {"_kg_hide-input":true}
countries_latest_state['DeathConfirmedRatio'] = (countries_latest_state.Fatalities + 1) / (
            countries_latest_state.ConfirmedCases + 1)
...
countries_in_march['IncreaseInMarch'] = countries_in_march.ConfirmedCases_0331 / (
            countries_in_march.ConfirmedCases_0301 + 1)
...
# %% [code] {"_kg_hide-input":true,"_kg_hide-output":true}
train_clean[
    'Geo#Country#Contintent'] = train_clean.Location + '#' + train_clean.Country_Region + '#' + train_clean.continent
...
# %% [code] {"scrolled":false,"_kg_hide-input":true,"_kg_hide-output":true}
...
confirmed_deltas[confirmed_deltas.DELTA != GLOBAL_DELTA].shape, confirmed_deltas[
    confirmed_deltas.DELTA != GLOBAL_DELTA].DELTA.mean()
...
for i, d in tqdm(enumerate(pd.date_range(add_days(TRAIN_END, 1), add_days(TEST_END, 1)))):
    ...
    for loc in confirmed_deltas.Location.values:
        ...
        daily_log_confirmed.loc[daily_log_confirmed.Location == loc, new_day] = daily_log_confirmed.loc[
                                                                                    daily_log_confirmed.Location == loc, last_day] + \
...
for i, d in tqdm(enumerate(pd.date_range(add_days(TRAIN_END, 1), add_days(TEST_END, 1)))):
    ...
    for loc in death_deltas.Location:
        ...
        daily_log_deaths.loc[daily_log_deaths.Location == loc, new_day] = daily_log_deaths.loc[
                                                                              daily_log_deaths.Location == loc, last_day] + \
                                                                          death_delta * DECAY ** i
