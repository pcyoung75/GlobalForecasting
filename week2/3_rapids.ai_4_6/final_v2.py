...
def get_cpmp_sub(save_oof=False, save_public_test=False):
    ...
    # Change date to absolute day (i.e., 1~365)
    ...
    data = pd.concat([train, test[test.day > max_test_val_day][train.columns]]).reset_index(drop=True)
...
print([np.sqrt(mean_squared_error(np.log1p(sub1[t].values), np.log1p(sub2[t].values))) for t in TARGETS])

sub_df = sub1.copy()

"""          ridge reg'=10   deep learning=9
                  log(sub1) + log(sub2) 
    sub_df = exp( --------------------- )
                            2   
"""
